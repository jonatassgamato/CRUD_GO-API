...
import (
	...
	"github.com/jonatassgamato/fullstack/api/models"
...
func TestCreatePost(t *testing.T) {
	...
	for _, v := range samples {
		...
		if v.statusCode == 201 {
			...
			assert.Equal(t, responseMap["author_id"], float64(v.authorID)) //just for both ids to have the same type
		}
...
func TestUpdatePost(t *testing.T) {
	...
	for _, v := range samples {
		...
		if v.statusCode == 200 {
			assert.Equal(t, responseMap["title"], v.title)
			assert.Equal(t, responseMap["content"], v.content)
			assert.Equal(t, responseMap["author_id"], float64(v.authorID)) //just to match the type of the json we receive thats why we used float64
		}
		if v.statusCode == 401 || v.statusCode == 422 || v.statusCode == 500 && v.errorMessage != "" {
			assert.Equal(t, responseMap["error"], v.errorMessage)
		}
